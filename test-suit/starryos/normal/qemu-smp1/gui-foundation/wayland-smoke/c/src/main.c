...
static int pump_roundtrip(struct TestState *state) {
    ...
    if (wl_event_loop_dispatch(wl_display_get_event_loop(state->server_display), 1) < 0) {
...
static int setup_wayland_pair(struct TestState *state) {
    ...
    trace_step("create socketpair transport");
    int socks[2];
    if (socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, socks) != 0) {
        fprintf(stderr, "FAIL: Wayland socketpair: %s\n", strerror(errno));
        return 1;
    }

    trace_step("create server client");
    state->server_client = wl_client_create(state->server_display, socks[0]);
    if (state->server_client == NULL) {
    ...
    trace_step("connect client display to fd");
    state->client_display = wl_display_connect_to_fd(socks[1]);
    if (state->client_display == NULL) {
        fprintf(stderr, "FAIL: wl_display_connect_to_fd returned NULL\n");
        wl_client_destroy(state->server_client);
        state->server_client = NULL;
        close(socks[1]);
        return 1;
    }
    return 0;
